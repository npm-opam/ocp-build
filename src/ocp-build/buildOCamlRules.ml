...
let add_info b lib options name =
  ...
  | "ocp::commit" ->
    Printf.bprintf b "let commit = %S\n"
      (let commit = find_git_commit lib.lib_src_dir.dir_fullname in
     ...
     it ? *)
      let needs_cmi =
        ...
        with NoSuchFileInDir _ ->
         ...
         ignore (add_mli_source b lib ptmp mli_file (set_bool options "ml" false) : unit);
              let cmi_file = find_dst_file dst_dir cmi_name in
         Some cmi_file
            | None -> None
          else
...
(*  BuildOCPPrinter.eprint_project "BuildOCamlRules.create" ptmp; *)
  ...
  Array.iter (fun lib ->
    try
      ...
      begin
        try
          let deps = linkdeps_option.get [lib.lib_options] in
          List.iter (fun name ->
            try
